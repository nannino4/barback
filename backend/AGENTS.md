...
This is a **monorepo-by-folder, not by git**: `backend/`, `frontend/`, and
`deploy/` are each their **own independent git repository** (there is no git repo
at the root). Commit, branch, and PR within the relevant subdirectory — never
assume a root-level git context. Each subdir has its own `docs/`,
`.github/workflows/`, and `.env.*` files.
