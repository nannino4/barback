...
if [[ "${SKIP_VALIDATE:-false}" != "true" ]]; then
  VITE_BUILD_MODE="$ENVIRONMENT" SKIP_NPM_CI="${SKIP_NPM_CI:-false}" bash scripts/ci-local.sh
fi
...
IMAGE_BASE="$DOCKER_HUB_USERNAME/barback-frontend"

docker build \
  --file Dockerfile \
  --target nginx \
  --build-arg VITE_BUILD_MODE="$ENVIRONMENT" \
